...
namespace nmGfx
{
    void Framebuffer::CreateDefault(Window* pWindow, int width, int height)
    {
        _width = width;
        _height = height;
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _gAlbedoSpec, 0);
        ...
        if(status != GL_FRAMEBUFFER_COMPLETE)
            ...
            printf("Framebuffer Complete. id:%i -- %i,%i\n", _id, _gPosition, _depthBuffer);
